...
module mm_ram #(
  ...
  logic [31:0] npu_shared_addr_src;
  logic       npu_all_done;
  ...
  assign npu_host_addr = (!data_we_i && transaction_q == T_PER && is_npu_addr(error_addr_q))
      ? error_addr_q[4:0]
      : data_addr_i[4:0];
  assign npu_host_wr_data = selected_write_byte(data_wdata_i, data_addr_i);
  assign npu_host_wr_en = data_req_i && data_we_i && is_npu_addr(data_addr_i) && data_be_i[data_addr_i[1:0]];
  assign npu_shared_addr_src = (!data_we_i && transaction_q == T_PER && is_npu_shared_addr(error_addr_q))
      ? error_addr_q
      : data_addr_i;
  assign npu_shared_addr = is_npu_shared_im_addr(npu_shared_addr_src)
      ? (NPU_IM_BASE_WORD_ADDR + ((npu_shared_addr_src - NPU_SHARED_IM_BASE_ADDR) >> 4))
      : ((npu_shared_addr_src - NPU_SHARED_UB_BASE_ADDR) >> 4);
  assign npu_shared_lane = npu_shared_addr_src[3:2];
  assign npu_shared_wr_data = data_wdata_i;
  assign npu_shared_wr_be = data_be_i;
  assign npu_shared_wr_en = data_req_i && data_we_i && is_npu_shared_addr(data_addr_i) && npu_shared_allow;
  assign npu_shared_rd_en = data_req_i && !data_we_i && is_npu_shared_addr(data_addr_i) && npu_shared_allow;
  assign npu_data_rdata = (transaction_q == T_PER && is_npu_shared_addr(error_addr_q))
      ? (npu_shared_allow ? npu_shared_rd_data : 32'h0)
      : {4{npu_host_rd_data}};
...
`endif
          ...
          perip_gnt       = 1'b1;
        end else if (is_npu_addr(data_addr_i) || is_npu_shared_addr(data_addr_i)) begin
          perip_gnt = 1'b1;
        end else begin
          // out of bounds write
        end
    ...
    (@(posedge clk_i) disable iff (~rst_ni)
     ...
     || data_addr_i == 32'h2000_0004
      ...
      || data_addr_i[31:16] == 16'h1600
       || is_npu_addr(data_addr_i)
       || is_npu_shared_addr(data_addr_i)))
...
`endif
  ...
  always_comb begin : read_mux
    ...
    end else if (transaction_q == T_PER) begin
      if (is_npu_addr(error_addr_q) || is_npu_shared_addr(error_addr_q)) begin
        data_rdata_mux = npu_data_rdata;
      end else if (error_addr_q == 32'h1500_1000) begin
        data_rdata_mux = timer_cnt_q;
      end
