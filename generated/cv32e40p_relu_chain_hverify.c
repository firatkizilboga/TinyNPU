...
static int32_t x_data[16] __attribute__((section(".data"))) = {
    1, -2, 3, -4, 5, -6, 7, -8, -2, 1, -1, 2, 0, 3, -3, 1
};

static TinyTensor x = {"x", x_data, TINY_DTYPE_INT16, 2, {4, 4, 1, 1}, 16};

static int32_t w0_data[16] __attribute__((section(".data"))) = {
    1, 0, 1, 0, 0, 1, 0, 1, 1, 1, 0, 0, 0, 0, 1, 1
};

static TinyTensor w0 = {"w0", w0_data, TINY_DTYPE_INT16, 2, {4, 4, 1, 1}, 16};

static int32_t h_data[16];

static TinyTensor h = {"h", h_data, TINY_DTYPE_INT16, 2, {4, 4, 1, 1}, 16};

static int32_t h_relu_data[16];

static TinyTensor h_relu = {"h_relu", h_relu_data, TINY_DTYPE_INT16, 2, {4, 4, 1, 1}, 16};

static int32_t w1_data[16] __attribute__((section(".data"))) = {
    1, 2, 0, 1, 0, 1, 1, 0, 2, 0, 1, 1, 1, 1, 0, 2
};

static TinyTensor w1 = {"w1", w1_data, TINY_DTYPE_INT16, 2, {4, 4, 1, 1}, 16};

static int32_t y_data[16];

static TinyTensor y = {"y", y_data, TINY_DTYPE_INT16, 2, {4, 4, 1, 1}, 16};

static int32_t h_expected_data[16] __attribute__((section(".data"))) = {
    4, 1, -3, -6, 12, 1, -3, -14, -3, 0, 0, 3, -3, 0, 1, 4
};

static TinyTensor h_expected = {"h", h_expected_data, TINY_DTYPE_INT16, 2, {4, 4, 1, 1}, 16};

static int32_t y_expected_data[16] __attribute__((section(".data"))) = {
    4, 9, 1, 4, 12, 25, 1, 12, 3, 3, 0, 6, 6, 4, 1, 9
};

static TinyTensor y_expected = {"y", y_expected_data, TINY_DTYPE_INT16, 2, {4, 4, 1, 1}, 16};

static uint32_t tinynpu_static_ub_image[16][TINY_BUFFER_WORDS_32] __attribute__((section(".data"))) = {
    ...
    {0x00000000u, 0x00010001u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u},
    {0x00020001u, 0x00010000u, 0x00000000u, 0x00000000u},
    {0x00010000u, 0x00000001u, 0x00000000u, 0x00000000u},
    {0x00000002u, 0x00010001u, 0x00000000u, 0x00000000u},
    {0x00010001u, 0x00020000u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u}
...
static uint32_t im_segment_000[4][TINY_BUFFER_WORDS_32] __attribute__((section(".data"))) = {
    {0x00000000u, 0x00000000u, 0x00880000u, 0xff000001u},
    {0x010001ffu, 0x00000100u, 0x00001000u, 0x20001800u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x10000000u}
};

static uint32_t im_segment_001[4][TINY_BUFFER_WORDS_32] __attribute__((section(".data"))) = {
    {0x00000000u, 0x00000000u, 0x00880000u, 0xff000001u},
    {0x010001ffu, 0x00000100u, 0x08001800u, 0x20001000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x00000000u},
    {0x00000000u, 0x00000000u, 0x00000000u, 0x10000000u}
};
...
int main(void)
{
    printf("TinyNPU bare-metal program: cv32e40p_relu_chain_hverify\n");
    load_ub_image(0u, tinynpu_static_ub_image, 16);
    load_im_image(0x8000u, im_segment_000, 4);
    load_im_image(0x8004u, im_segment_001, 4);
    printf("NpuSegment: segment_000\n");
    write_tensor_to_npu(&x, 0x0018u, "A", 2, 8);
    if (npu_run(0x8000u) != 0) return EXIT_FAILURE;
    read_role_c_tensor(&h, 0x0010u, 2);
    printf("Intermediate h after segment_000:\n");
    print_tensor(&h);
    printf("HostOp relu: relu_h\n");
    host_relu(&h_relu, &h);
    printf("Intermediate h_relu after host relu:\n");
    print_tensor(&h_relu);
    printf("NpuSegment: segment_001\n");
    write_tensor_to_npu(&h_relu, 0x0010u, "A", 2, 8);
    {
        uint8_t dbg_bytes[TINY_MMVR_BYTES];
        for (int dbg_row = 0; dbg_row < 8; ++dbg_row) {
            if (npu_read_mem_word((uint16_t)(0x0010u + dbg_row), dbg_bytes) != 0) {
                printf("debug readback failed at A word %d\n", dbg_row);
                return EXIT_FAILURE;
            }
            printf("A word %d:", dbg_row);
            for (int lane = 0; lane < TINY_ARRAY_SIZE; ++lane) {
                uint16_t packed_lane = (uint16_t)dbg_bytes[lane * 2] | ((uint16_t)dbg_bytes[lane * 2 + 1] << 8);
                printf(" 0x%04x", (unsigned)packed_lane);
            }
            printf("\n");
        }
    }
    if (npu_run(0x8004u) != 0) return EXIT_FAILURE;
    read_role_c_tensor(&y, 0x0018u, 2);
    printf("Final outputs:\n");
    print_tensor(&y);
    if (!tensor_matches_expected(&h, &h_expected)) {
        printf("verification failed: mid_h (h)\n");
        printf("meta actual dtype=%d elems=%d expected dtype=%d elems=%d\n", h.dtype, h.elem_count, h_expected.dtype, h_expected.elem_count);
        print_tensor(&h);
        print_tensor(&h_expected);
        return EXIT_FAILURE;
    }
    if (!tensor_matches_expected(&y, &y_expected)) {
        printf("verification failed: final_y (y)\n");
        printf("meta actual dtype=%d elems=%d expected dtype=%d elems=%d\n", y.dtype, y.elem_count, y_expected.dtype, y_expected.elem_count);
        print_tensor(&y);
        print_tensor(&y_expected);
        return EXIT_FAILURE;
    }
