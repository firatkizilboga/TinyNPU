...
def compute_fused_params(w_scale: float, a_scale: float, out_scale: float) -> tuple[int, int]:
    """Compute TinyNPU PPU multiplier/shift from real-valued quant scales.

    This is the shared PyTorch-side bridge:
    effective_scale = (w_scale * a_scale) / out_scale
    out ~= (acc * multiplier) >> shift
    """
    if out_scale <= 0:
        raise ValueError(f"out_scale must be positive, got {out_scale}.")
    if w_scale < 0 or a_scale < 0:
        raise ValueError(f"w_scale and a_scale must be non-negative, got {w_scale} and {a_scale}.")
    effective_scale = (float(w_scale) * float(a_scale)) / float(out_scale)
