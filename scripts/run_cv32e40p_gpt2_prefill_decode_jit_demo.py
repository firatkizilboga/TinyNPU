...
from run_cv32e40p_prefill_transformer_block_jit_demo import (  # noqa: E402
    _fp16_roundtrip,
    _layernorm_runtime_approx,
)
...
def _build_shared_state(
    *,
    ...
    state: dict[str, object] = {
        ...
        "ln1_wb": np.stack(
            [
                rng.uniform(0.5, 1.5, size=(d_model,)).astype(np.float32),
                _rand_f32(rng, (d_model,), low=-0.1, high=0.1),
            ],
            axis=0,
        ).astype(np.float32),
        "ln2_wb": np.stack(
        ...
        "w_q": [_rand_i16(rng, (d_model, d_head)) for _ in range(n_heads)],
        "w_k": [_rand_i16(rng, (d_model, d_head)) for _ in range(n_heads)],
        "w_v": [_rand_i16(rng, (d_model, d_head)) for _ in range(n_heads)],
        "b_q": [_rand_f32(rng, (1, d_head), low=-0.05, high=0.05) for _ in range(n_heads)],
        "b_k": [_rand_f32(rng, (1, d_head), low=-0.05, high=0.05) for _ in range(n_heads)],
        "b_v": [_rand_f32(rng, (1, d_head), low=-0.05, high=0.05) for _ in range(n_heads)],
        "w_o": _rand_i16(rng, (attn_dim, d_model)),
        "b_o": _rand_f32(rng, (1, d_model), low=-0.05, high=0.05),
        "w_fc": _rand_i16(rng, (d_model, ffn_dim)),
        "b_fc": _rand_f32(rng, (1, ffn_dim), low=-0.05, high=0.05),
        "w_proj": _rand_i16(rng, (ffn_dim, d_model)),
        "b_proj": _rand_f32(rng, (1, d_model), low=-0.05, high=0.05),
    }
...
def _reference_prefill(
    ...
    *,
    ...
    ln1_wb = np.array(state["ln1_wb"], dtype=np.float32, copy=True)
    ln2_wb = np.array(state["ln2_wb"], dtype=np.float32, copy=True)
    w_q = [np.array(w, dtype=np.int16, copy=True) for w in state["w_q"]]
    w_k = [np.array(w, dtype=np.int16, copy=True) for w in state["w_k"]]
    w_v = [np.array(w, dtype=np.int16, copy=True) for w in state["w_v"]]
    b_q = [_quantize_bias_fp32(np.array(b, dtype=np.float32, copy=True), out_scale=act_scale) for b in state["b_q"]]
    b_k = [_quantize_bias_fp32(np.array(b, dtype=np.float32, copy=True), out_scale=act_scale) for b in state["b_k"]]
    b_v = [_quantize_bias_fp32(np.array(b, dtype=np.float32, copy=True), out_scale=act_scale) for b in state["b_v"]]
    w_o = np.array(state["w_o"], dtype=np.int16, copy=True)
    b_o = _quantize_bias_fp32(np.array(state["b_o"], dtype=np.float32, copy=True), out_scale=act_scale)
    w_fc = np.array(state["w_fc"], dtype=np.int16, copy=True)
    b_fc = _quantize_bias_fp32(np.array(state["b_fc"], dtype=np.float32, copy=True), out_scale=act_scale)
    w_proj = np.array(state["w_proj"], dtype=np.int16, copy=True)
    b_proj = _quantize_bias_fp32(np.array(state["b_proj"], dtype=np.float32, copy=True), out_scale=act_scale)
...
def _reference_decode(
    ...
    *,
    ...
    cache_len = int(np.array(state["x_prompt_in"]).shape[0] + 1)
    ln1_wb = np.array(state["ln1_wb"], dtype=np.float32, copy=True)
    ln2_wb = np.array(state["ln2_wb"], dtype=np.float32, copy=True)
    w_q = [np.array(w, dtype=np.int16, copy=True) for w in state["w_q"]]
    w_k = [np.array(w, dtype=np.int16, copy=True) for w in state["w_k"]]
...
def build_prefill_artifact(
    *,
    ...
    tensors: dict[str, TensorSpec] = {
        "x_in": TensorSpec("x_in", (prompt_len, d_model), DType.FLOAT32, TensorKind.CONSTANT, data=np.array(state["x_prompt_in"], dtype=np.float32, copy=True)),
        "ln1_wb": TensorSpec("ln1_wb", (2, d_model), DType.FLOAT32, TensorKind.CONSTANT, data=np.array(state["ln1_wb"], dtype=np.float32, copy=True)),
        "ln2_wb": TensorSpec("ln2_wb", (2, d_model), DType.FLOAT32, TensorKind.CONSTANT, data=np.array(state["ln2_wb"], dtype=np.float32, copy=True)),
        "w_o": TensorSpec("w_o", (attn_dim, d_model), DType.INT16, TensorKind.CONSTANT, data=np.array(state["w_o"], dtype=np.int16, copy=True)),
        "b_o": TensorSpec("b_o", (1, d_model), DType.INT32, TensorKind.CONSTANT, data=_quantize_bias_fp32(np.array(state["b_o"], dtype=np.float32, copy=True), out_scale=act_scale)),
        "w_fc": TensorSpec("w_fc", (d_model, ffn_dim), DType.INT16, TensorKind.CONSTANT, data=np.array(state["w_fc"], dtype=np.int16, copy=True)),
        "b_fc": TensorSpec("b_fc", (1, ffn_dim), DType.INT32, TensorKind.CONSTANT, data=_quantize_bias_fp32(np.array(state["b_fc"], dtype=np.float32, copy=True), out_scale=act_scale)),
        "w_proj": TensorSpec("w_proj", (ffn_dim, d_model), DType.INT16, TensorKind.CONSTANT, data=np.array(state["w_proj"], dtype=np.int16, copy=True)),
        "b_proj": TensorSpec("b_proj", (1, d_model), DType.INT32, TensorKind.CONSTANT, data=_quantize_bias_fp32(np.array(state["b_proj"], dtype=np.float32, copy=True), out_scale=act_scale)),
        "x_norm1": TensorSpec("x_norm1", (prompt_len, d_model), DType.INT16, TensorKind.INTERMEDIATE),
        "x_norm1_q": TensorSpec("x_norm1_q", (prompt_len, d_model), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
        "attn_cat": TensorSpec("attn_cat", (prompt_len, attn_dim), DType.INT16, TensorKind.INTERMEDIATE),
        "attn_cat_a": TensorSpec("attn_cat_a", (prompt_len, attn_dim), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
        "o_int": TensorSpec("o_int", (prompt_len, d_model), DType.INT16, TensorKind.INTERMEDIATE),
        "o_f": TensorSpec("o_f", (prompt_len, d_model), DType.FLOAT32, TensorKind.INTERMEDIATE),
        "resid1": TensorSpec("resid1", (prompt_len, d_model), DType.FLOAT32, TensorKind.INTERMEDIATE),
        "x_norm2": TensorSpec("x_norm2", (prompt_len, d_model), DType.INT16, TensorKind.INTERMEDIATE),
        "x_norm2_q": TensorSpec("x_norm2_q", (prompt_len, d_model), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
        "ffn_fc_int": TensorSpec("ffn_fc_int", (prompt_len, ffn_dim), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
        "ffn_out_int": TensorSpec("ffn_out_int", (prompt_len, d_model), DType.INT16, TensorKind.INTERMEDIATE),
        "ffn_out_f": TensorSpec("ffn_out_f", (prompt_len, d_model), DType.FLOAT32, TensorKind.INTERMEDIATE),
        "out": TensorSpec("out", (prompt_len, d_model), DType.FLOAT32, TensorKind.OUTPUT, is_final_output=True),
    }
    ...
    for head_idx in range(n_heads):
        tensors[f"w_q_h{head_idx}"] = TensorSpec(f"w_q_h{head_idx}", (d_model, d_head), DType.INT16, TensorKind.CONSTANT, data=np.array(state["w_q"][head_idx], dtype=np.int16, copy=True))
        tensors[f"w_k_h{head_idx}"] = TensorSpec(f"w_k_h{head_idx}", (d_model, d_head), DType.INT16, TensorKind.CONSTANT, data=np.array(state["w_k"][head_idx], dtype=np.int16, copy=True))
        tensors[f"w_v_h{head_idx}"] = TensorSpec(f"w_v_h{head_idx}", (d_model, d_head), DType.INT16, TensorKind.CONSTANT, data=np.array(state["w_v"][head_idx], dtype=np.int16, copy=True))
        tensors[f"b_q_h{head_idx}"] = TensorSpec(f"b_q_h{head_idx}", (1, d_head), DType.INT32, TensorKind.CONSTANT, data=_quantize_bias_fp32(np.array(state["b_q"][head_idx], dtype=np.float32, copy=True), out_scale=act_scale))
        tensors[f"b_k_h{head_idx}"] = TensorSpec(f"b_k_h{head_idx}", (1, d_head), DType.INT32, TensorKind.CONSTANT, data=_quantize_bias_fp32(np.array(state["b_k"][head_idx], dtype=np.float32, copy=True), out_scale=act_scale))
        tensors[f"b_v_h{head_idx}"] = TensorSpec(f"b_v_h{head_idx}", (1, d_head), DType.INT32, TensorKind.CONSTANT, data=_quantize_bias_fp32(np.array(state["b_v"][head_idx], dtype=np.float32, copy=True), out_scale=act_scale))
        tensors.update(
            make_native_int16_kv_cache_specs(
...
def build_decode_artifact(
    *,
    ...
    prefill_ref = _reference_prefill(
        state,
        d_head=d_head,
        n_heads=n_heads,
        act_scale=act_scale,
    ...
    tensors: dict[str, TensorSpec] = {
        "x_in": TensorSpec("x_in", (1, d_model), DType.FLOAT32, TensorKind.CONSTANT, data=np.array(state["x_decode_in"], dtype=np.float32, copy=True)),
        "ln1_wb": TensorSpec("ln1_wb", (2, d_model), DType.FLOAT32, TensorKind.CONSTANT, data=np.array(state["ln1_wb"], dtype=np.float32, copy=True)),
        "ln2_wb": TensorSpec("ln2_wb", (2, d_model), DType.FLOAT32, TensorKind.CONSTANT, data=np.array(state["ln2_wb"], dtype=np.float32, copy=True)),
        "w_o": TensorSpec("w_o", (attn_dim, d_model), DType.INT16, TensorKind.CONSTANT, data=np.array(state["w_o"], dtype=np.int16, copy=True)),
        "b_o": TensorSpec("b_o", (1, d_model), DType.INT32, TensorKind.CONSTANT, data=_quantize_bias_fp32(np.array(state["b_o"], dtype=np.float32, copy=True), out_scale=act_scale)),
        ...
        "w_proj": TensorSpec("w_proj", (ffn_dim, d_model), DType.INT16, TensorKind.CONSTANT, data=np.array(state["w_proj"], dtype=np.int16, copy=True)),
        "b_proj": TensorSpec("b_proj", (1, d_model), DType.INT32, TensorKind.CONSTANT, data=_quantize_bias_fp32(np.array(state["b_proj"], dtype=np.float32, copy=True), out_scale=act_scale)),
        "x_norm1": TensorSpec("x_norm1", (1, d_model), DType.INT16, TensorKind.INTERMEDIATE),
        "x_norm1_q": TensorSpec("x_norm1_q", (1, d_model), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
        "attn_cat": TensorSpec("attn_cat", (1, attn_dim), DType.INT16, TensorKind.INTERMEDIATE),
        "attn_cat_a": TensorSpec("attn_cat_a", (1, attn_dim), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
        ...
        "x_norm2": TensorSpec("x_norm2", (1, d_model), DType.INT16, TensorKind.INTERMEDIATE),
        "x_norm2_q": TensorSpec("x_norm2_q", (1, d_model), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
        "ffn_fc_int": TensorSpec("ffn_fc_int", (1, ffn_dim), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
    ...
    for head_idx in range(n_heads):
        ...
        # Standalone decode smoke currently seeds the base KV cache from the Python
        # prefill reference. This is numerically equivalent to a real handoff, but
        # it is not yet a runtime-level prefill.hex -> decode.hex cache transfer.
        k_prefill = np.array(prefill_ref["k_heads"][head_idx], dtype=np.int16, copy=True)
        v_prefill = np.array(prefill_ref["v_heads"][head_idx], dtype=np.int16, copy=True)
