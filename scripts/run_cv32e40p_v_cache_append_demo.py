...
def build_program() -> tuple[TinyNPUProgram, np.ndarray]:
    ...
    program.declare_data("v_cache", np.zeros((16, 16), dtype=np.int16), precision=PrecisionMode.INT16, role="B")
...
def _emit_program_source(program_name: str, program: TinyNPUProgram, expected_cache: np.ndarray) -> str:
    ...
    decls.append(
        ...
        f'    {{.name = "v_cache", .data = {symbol}_cache_data, .dtype = TNPU_DTYPE_INT16, .rank = 2, .shape = {{16, 16, 1, 1}}, .elem_count = 256}},\n'
        f'    {{.name = "v_cache_expected", .data = {symbol}_cache_expected_data, .dtype = TNPU_DTYPE_INT16, .rank = 2, .shape = {{16, 16, 1, 1}}, .elem_count = 256}}\n'
        "};"
    )
    decls.append(
        "static const TnpuVerifyOp "
        f"{symbol}_verify_ops[1] = {{\n"
        '    {.label = "v_cache_append", .actual_tensor_idx = 0, .expected_tensor_idx = 1, .is_final_output = 1}\n'
        "};"
    )
    decls.append(
        "static const TnpuOp "
...
def main() -> int:
    program_name = "cv32e40p_v_cache_append_demo_v2"
