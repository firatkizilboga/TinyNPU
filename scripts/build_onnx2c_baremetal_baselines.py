...
def build_model(model: str, program_name: str, *, prompt: str, prompt_len: int) -> tuple[Path, Path, Path, Path, Path]:
    ...
    func_name = "onnx2c_conv4" if model == "conv" else "onnx2c_tiny_lm"
...
def main() -> int:
    ...
    parser.add_argument("--model", choices=["conv", "tiny_lm"], default="conv")
    ...
    program_name = args.program_name or ("third_party_onnx2c_conv4" if args.model == "conv" else "third_party_onnx2c_tiny_lm")
