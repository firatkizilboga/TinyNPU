...
def build_plan(
    *,
    ...
    tensors = {
        ...
        "q_rope_q": TensorSpec("q_rope_q", (1, d_head), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
        ...
        "probs_q": TensorSpec("probs_q", (1, token_count), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
        ...
        "ffn_hidden_q": TensorSpec("ffn_hidden_q", (1, ffn_dim), DType.INT16, TensorKind.INTERMEDIATE, metadata={"storage_role": "A"}),
    ...
    for token_idx, token_name in zip(token_indices, token_names):
        lhs_k = _rand_i16(rng, (1, d_model))
        rhs_k = _rand_i16(rng, (d_model, d_head))
        lhs_v = _rand_i16(rng, (1, d_model))
        rhs_v = _rand_i16(rng, (d_model, d_head))
    ...
    plan.add_verification_step("probs_q", "probs_q")
    plan.add_verification_step("o_int", "o_int")
...
def build_artifact(**kwargs):
    ...
    token_count = int(kwargs["token_count"])
    ...
    token_indices = list(range(token_count))
    token_names = [f"t{idx}" for idx in token_indices]
    k_cache = np.zeros((d_head, token_count), dtype=np.int16)
    v_cache = np.zeros((token_count, d_head), dtype=np.int16)
    for token_idx, token_name in zip(token_indices, token_names):
        lhs_k = np.array(tensors[f"lhs_k_{token_name}"].data, dtype=np.int16, copy=True)
        rhs_k = np.array(tensors[f"rhs_k_{token_name}"].data, dtype=np.int16, copy=True)
        lhs_v = np.array(tensors[f"lhs_v_{token_name}"].data, dtype=np.int16, copy=True)
        rhs_v = np.array(tensors[f"rhs_v_{token_name}"].data, dtype=np.int16, copy=True)
        k_cache[:, token_idx] = golden.matmul(lhs_k, rhs_k, out_dtype=DType.INT16)[0]
        v_cache[token_idx, :] = golden.matmul(lhs_v, rhs_v, out_dtype=DType.INT16)[0]
    ...
    wanted = {
        "scores": np.array(expected["scores"], copy=True),
        "probs_q": np.array(expected["probs_q"], copy=True),
        "o_int": np.array(expected["o_int"], copy=True),
        "out": np.array(expected["out"], copy=True),
    }
...
def main() -> int:
    ...
    program_name = (
        f"cv32e40p_decoder_block_d{args.d_model}_h{args.d_head}"
        f"_f{args.ffn_dim}_t{args.token_count}_s{args.seed}_v2"
    )
