...
def build_program() -> tuple[TinyNPUProgram, np.ndarray]:
    ...
    expected_out = np.clip(query.astype(np.int32) @ token1.astype(np.int32), -32768, 32767).astype(np.int16)
...
def _emit_program_source(program_name: str, program: TinyNPUProgram, expected_out: np.ndarray) -> str:
    symbol = _sanitize(program_name)
    binary = program.compile()
    out = program.symbols["out"]

    decls: list[str] = []
    decls.append(_emit_i32_array(f"{symbol}_out_data", np.zeros(expected_out.shape, dtype=np.int32), section_data=False))
    decls.append(_emit_i32_array(f"{symbol}_out_expected_data", expected_out.astype(np.int32), section_data=True))
    ...
    decls.append(
        ...
        f'    {{.tensor_idx = 0, .addr = 0x{out.addr:04x}u, .precision = 2, .role = "C"}}\n'
        "};"
    )
    decls.append(
    ...
    decls.append(
        ...
        f'    {{.name = "out", .data = {symbol}_out_data, .dtype = TNPU_DTYPE_INT16, .rank = 2, .shape = {{8, 8, 1, 1}}, .elem_count = 64}},\n'
        f'    {{.name = "out_expected", .data = {symbol}_out_expected_data, .dtype = TNPU_DTYPE_INT16, .rank = 2, .shape = {{8, 8, 1, 1}}, .elem_count = 64}}\n'
        "};"
    )
    decls.append(
        "static const TnpuVerifyOp "
        f"{symbol}_verify_ops[1] = {{\n"
        '    {.label = "cache_consume", .actual_tensor_idx = 0, .expected_tensor_idx = 1, .is_final_output = 1}\n'
        "};"
    )
    decls.append(
        "static const TnpuOp "
...
def main() -> int:
    program_name = "cv32e40p_b_consumer_demo_v2"
    ...
    program, expected_out = build_program()
    GENERATED_DIR.mkdir(exist_ok=True)
    program_path.write_text(_emit_program_source(program_name, program, expected_out))
    ...
    print(f"consumer_b_word_offset=8")
    print(f"out_addr=0x{program.symbols['out'].addr:04x}")
    print(f"expected_checksum={int(expected_out.astype(np.int64).sum())}")
    print(proc.stdout)
    return 0
